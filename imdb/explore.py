...
long_prompts = []
with open("imdb/long_prompts.txt", "r") as file_:
    for line in file_.readlines():
        long_prompts.append(line.strip())

short_prompts = []
with open("imdb/short_prompts.txt", "r") as file_:
    for line in file_.readlines():
        short_prompts.append(line.strip())

short_prompts_llmlingua2 = []
with open("imdb/short_prompts_llmlingua2.txt", "r") as file_:
    for line in file_.readlines():
        short_prompts_llmlingua2.append(line.strip())

avg_len_long = sum(map(len, long_prompts)) / len(long_prompts)
avg_len_short = sum(map(len, short_prompts)) / len(short_prompts)
avg_len_llmlingua2 = sum(map(len, short_prompts_llmlingua2)) / len(short_prompts_llmlingua2)
print("Average length (long prompts):", avg_len_long)
print("Average length (short prompts):", avg_len_short)
print("Average length (LLMLingua-2 prompts):", avg_len_llmlingua2)
